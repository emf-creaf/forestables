...
+-----------------+--------------------------+------------------------+-----------------------------------------------------------------------------+
| subclass        | *Subclase*               | Subclase               | Code that determines the type of plot based on the location of the "rejon". |
+-----------------+--------------------------+------------------------+-----------------------------------------------------------------------------+
...
+---------------------+--------------------+-----------------------+------------------------------------------------------------+
| tree_id             | ARBOL              | nArbol                | number of trees, id                                        |
+---------------------+--------------------+-----------------------+------------------------------------------------------------+
...
We add a new variable called *density_factor.* It is a variable i similar to the "FAC" variable in the inventory, which corresponds to the Expansion Factor per hectare of the unit values of different dendrometric parameters of the measured foot. IFN follows a methods called variable-radius plot sampling. Smaller trees are measured at smaller radii of the plot, while larger trees are measured within the larger radii as follows.

![](images/clipboard-3909002882.png){width="529"}

| DBH interval | Density value |
|--------------|---------------|
| [0, 12.5)    | 127.3239546   |
...
\
The final variables provided are: "id_unique_code", "province_code", "class", "subclass", "plot", "sp_code", "sp_name", "tree", "wood_quality", "cubing_form", "tree_ifn2", "tre_ifn3", "tree_ifn4", "dbh", "height", "density_factor"
...
The final variables provided are: "id_unique_code", "province_code", "class", "subclass", "plot", "sp_name", "sp_code", "height", "cover"
...
To uniform all information and separate each record in a different row some assumptions are made. Additionally, new variables are added (dbh, density_factor, n).

-   n ( actual density in trees/ ha ) is calculated by multiplying nº trees \* *density_factor.*\
...
-   density_factor= FAC Expansion Factor per hectare which is always 127.3239546

-   dbh *=* diameter at breast height in cm

-   height = for CatDes 1,2 and 3 in cm

-   n = density in individuals per ha calculated as density_factor\* NumPies
...
The final variables given are: "id_unique_code", "province_code", "class", "subclass", "plot", "sp_code", "sp_name", "dbh", "height", "n", "density_factor"
...
Additionally, two variables are added to indicate coordinate system (*coord_sys)* and CRS - ESPG code (*crs*). They are called and *crs*. We also add a variable indicaruing version of IFN (*version*).\
**\
For IFN2:**\
\
...
Values of variables *coord_sys* and *huso* are assigned following the technical documents of IFN. Crs is obtained from both. When *huso* is missing (IFN3) , it is assumed that its value is Huso = 30. (This will be corrected in near future).\
\
Final variables are "id_unique_plot", "country", "ca_name_original", "province_code", "province_name_original", "plot", "class", "subclass", "coord_sys", "year", "version", "type", "aspect", "slope"\
...
Final variables are "id_unique_plor", "plot", "dep", "year", "tree", "espar", "sp_code", "sp_name", "status", "status5", "dbh", "height", "density_factor"
...
Final variables are "id_unique_code", "plot", "dep", "year", "sp_code", "sp_name", "cover", "height", "growth_form"
...
Here same process of SHURB table process is performed but in the end only those with a *growth_form* of "*tree*" are selected. Note that this data does not have dbh, therefore, dbh = NA.
...
Variables regarding to herb or woody vegetation (LIGN ) are collected it undersatory table.\
\
The final variables provided are: "id_unique_code", "province_code", "class", "subclass", "plot", "sp_name", "sp_code", "height", "cover"
...
Final variables provided are : "id_unique_code", "year", "state_code", "county_code", "plot", "tree_id", "status", "dbh", "height", "sp_name", "sp_code", "density_factor".
...
All information about is at subplot level.
...
Final variables provided are : "id_unique_code", "year", "state_code", "county_code", "plot", "subplot", "sp_name", "height", "cover", "growth_form", "sp_code"
...
Final variables provided are : "id_unique_code", "year", "state_code", "county_code", "plot", "subplot", "sp_name", "height", "cover", "growth_form","growth_form_code", "sp_code"
...
Data is obtained from both the last available year with the required information for the plot and the year of interest. If data for quered year is not available, we provide the last recorded value. Note that in most cases information on plot condition is collected once, when the plot is set.
...
"id_unique_code", "year", "state_code", "county_code", "plot", "subplot", "slope_subplot", "aspect_subplot", "macro_cond", "subplot_cond", "micro_cond", "subplot_status", "psveg_subplot_status"
