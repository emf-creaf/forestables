...
uniformize the data between inventories has been done.
  ...
  * IFN unique tree variables:  
    `cubing_form`, `quality_wood`. Also, IFN lack the `status` variable.
...
common_tree_names <- c(ifn_tree_names, ffi_tree_names, fia_tree_names) |>
  magrittr::extract(
    c(ifn_tree_names, ffi_tree_names, fia_tree_names) |>
      duplicated() |> which()
  ) |>
  unique()
...
common_understory_names <- c(ifn_understory_names, ffi_understory_names, fia_understory_names) |>
  magrittr::extract(
    c(ifn_understory_names, ffi_understory_names, fia_understory_names) |>
      duplicated() |> which()
  ) |>
  unique()
