...
north_spain_plots_2012 <- north_spain_plots |>
  filter(INVYR == 2012)

ggplot(west_coast_map) +
  geom_sf() +
  geom_sf(aes(color = STATEAB, alpha = 0.1), data = north_spain_plots_2012) +
  scale_color_manual(values = hcl.colors(9, palette = "Zissou 1"))

fia_filter_list_2012 <-
  show_plots_from("FIA", folder = fia_folder, states = north_spain_provinces) |>
  filter(INVYR == 2012) |>
  create_filter_list()
