...
```{r ffi_mediterranean_deps, eval = TRUE}
...
```{r ffi_selection_2012, eval = TRUE}
...
```{r ffi_department_polygons, eval = TRUE}
...
```{r ffi_filter_list_creation, eval = TRUE}
...
ffi_data_2012 |>
  ...
  unnest("tree") |>
  # calculate density by species in each plot
...
for example, the most abundant species and their density (trees/ha):
