...
Also, if we obtain the state polygons with the `rnaturalearth` library, we
can visualize the plots:

```{r fia_states_polygons, fig.width=6, fig.align="center"}
# USA map for plotting
hawaii_map <- ne_states(geounit = "United States of America") |>
  select(state_code = "iso_3166_2", state_name = "gn_name") |>
  mutate(state_code = str_remove(state_code, "US-")) |>
  filter(state_code %in% "HI")

ggplot(hawaii_map) +
  geom_sf() +
  geom_sf(aes(color = STATEAB, alpha = 0.4), data = hawaii_plots_2019) +
...
fia_data_2019 |>
  ...
  # let's plot
  ggplot() +
  geom_sf(data = hawaii_map, alpha = 0, linewidth = 0.3) +
  geom_sf(aes(geometry = geometry, color = dbh), size = 2.2, alpha = 0.8) +
  coord_sf(xlim = c(-160.6, -154.5), ylim = c(18.5, 22.5)) +
  scale_color_gradientn(colors = hcl.colors(360, "PinkYl", rev = TRUE)) +
  facet_wrap(~sp_name, ncol = 3) +
