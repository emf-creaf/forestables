...
data_2012 |>
  ...
  dplyr::summarise(HT = mean(HT, na.rm = TRUE), n = n()) |>
  dplyr::filter(!is.nan(HT)) |>
  dplyr::group_by(SP_NAME) |>
  dplyr::mutate(n = n()) |>
  dplyr::filter(n > 25) |> 
  ggplot() +
  geom_sf(data = france_map, alpha = 0, linewidth = 0.8) +
  geom_sf(aes(color = HT), size = 2.2, alpha = 0.8) +
  scale_color_gradientn(colors = hcl.colors(360, "Dark Mint", rev = TRUE)) +
  facet_wrap(~ SP_NAME) +
  theme_minimal()
```
